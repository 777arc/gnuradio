...
#include <cstdint>
namespace {
// Boost.Locale is a compiled Boost library and is not part of the WASM sysroot,
// yet it is used in exactly one place (RDS RadioText is ISO-8859-2). Provide a
// self-contained ISO-8859-2 -> UTF-8 conversion so the WASM build needs no extra
// Boost component. Bytes < 0xA0 pass through unchanged.
inline std::string rds_iso8859_2_to_utf8(const std::string& in) {
    static const uint16_t high[96] = {
    ...
    for (unsigned char c : in) {
        uint32_t cp = (c < 0xA0) ? c : high[c - 0xA0];
        if (cp < 0x80) {
...
void parser_impl::send_message(long msgtype, std::string msgtext) {
    ...
    std::string utf8text = rds_iso8859_2_to_utf8(msgtext);
#else
